...
class GrafanaDashboardContractTest {
    ...
    fun `admin console exposes shard scaling without producer stress controls`() {
        ...
        assertTrue(adminJs.contains("async function initializeAdmin"))
        assertTrue(adminJs.contains("await loadAdminSession(adminState.authHeader)"))
        assertTrue(adminJs.contains("await refreshAdminGroups()"))
        assertTrue(adminJs.contains("showAdminError(error.message)"))
        ...
        assertFalse(adminJs.contains("""loadAdminSession(adminState.authHeader).then(refreshAdminGroups).catch(() => redirectToSignIn("admin"))"""))
