...
Coordinator server는 Prometheus registry가 있을 때 Spring Boot Actuator `/actuator/prometheus`로 Prometheus 형식 metric을 계속 노출할 수 있다. 다만 repository가 제공하는 Grafana dashboard는 더 이상 Prometheus에 의존하지 않는다. smoke stack은 API latency/request-rate 패널을 위해 Loki audit log datasource를 provision하고, shard/group/member/message 상태 패널은 coordinator monitoring API용 Infinity datasource를 사용한다. Grafana는 custom monitoring console을 iframe으로 embed하지 않고 Grafana-managed datasource를 통해 coordinator monitoring API를 직접 호출해야 한다. Coordinator API credential은 Grafana datasource provisioning에만 있어야 하며 dashboard panel URL에 하드코딩하지 않는다.
