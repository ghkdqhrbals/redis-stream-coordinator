...
`targetShardCount=0`일 때 2번은 producer routing이 `shardCount=0`과 빈 shard list를 반환한다는 뜻이다. 그래도 Terraform은 모든 제거 대상 stream shard가 모든 Redis consumer group 기준으로 drain될 때까지 monitoring state를 poll해야 stream retired로 간주할 수 있다.
