...
| Scale-in 중 live member가 모두 expire됨 | revoke ack를 보낼 heartbeat 대상이 없다. Coordinator는 migration을 Redis-level drain check로 진행하고, 제거 대상 shard의 모든 Redis group이 `pending=0`과 known `lag=0`을 보고할 때만 완료한다 |
