...
* `coordinator-server` control plane with group creation, heartbeat reconciliation, sticky assignment, revoke-before-assign, scale migration, rollback, monitoring APIs with conflict retry, scheduled coordinator event loop, Redis-backed state mutex, ACL, audit logging, admin mutation rate limiting, and Micrometer metrics.
