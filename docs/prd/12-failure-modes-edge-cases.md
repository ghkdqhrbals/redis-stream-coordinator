...
| Deployment previously used memory store and is restarted with Redis store after physical shards already exist | Physical shard keys exist, but stream-level coordinator metadata is missing, so initial heartbeats still return `UNKNOWN_MEMBER_ID` | Recover through `POST /coord/v1/streams/{streamPrefix}/adopt` with the exact existing shard count. The endpoint verifies every physical shard key exists, records only stream metadata, and lets first heartbeats register runtime groups |
